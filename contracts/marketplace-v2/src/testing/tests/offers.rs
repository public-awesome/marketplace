use crate::{
    ...
    state::{Config, ExpirationInfo, KeyString, Offer, PriceRange},
    testing::{
        helpers::{
...
fn try_set_simple_offer() {
    ...
    let offer_price = coin(2_000_000, NATIVE_DENOM);
    ...
    assert_error(
        ...
        ContractError::InsufficientFunds {
            expected: offer_price,
        }
        .to_string(),
    );

    // Create offer with invalid denom fails
    let offer_price = coin(2_000_000, JUNO_DENOM);
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        price: offer_price.clone(),
    ...
    // Create offer with price too low fails
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
    ...
    let offer = router
        ...
        .unwrap()
        .unwrap();
    assert_eq!(offer.order_info.price, price);
    assert_eq!(offer.order_info.creator, bidder);
    assert_eq!(offer.order_info.asset_recipient, None);
    assert_eq!(offer.order_info.finders_fee_bps, None);
    assert_eq!(offer.order_info.expiration_info, None);
    ...
    assert_error(
        ...
        ContractError::EntityExists(format!(
            "offer {}",
            Offer::build_key(&collection, &token_id.to_string(), &bidder).to_string()
        ))
        .to_string(),
    );
...
pub fn try_set_complex_offer() {
    ...
    // Create offer with finder sender fails
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
    ...
    // Create offer with finders fee above 100% fails
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        price: coin(native_denom_price_range.min.u128(), NATIVE_DENOM),
    ...
    // Create offer with expiration info fails when no fee is paid
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        price: coin(native_denom_price_range.min.u128(), NATIVE_DENOM),
    ...
    // Create offer with expiration too soon fails
    let set_offer = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        price: coin(native_denom_price_range.min.u128(), NATIVE_DENOM),
    ...
    // Create offer with all valid parameters succeeds
    let price = coin(native_denom_price_range.min.u128(), NATIVE_DENOM);
    ...
    assert_eq!(offer.order_info.creator, bidder);
    assert_eq!(
        offer.order_info.asset_recipient,
        Some(asset_recipient.clone())
    );
    assert_eq!(offer.order_info.finders_fee_bps, Some(finders_fee_bps));
    assert_eq!(offer.order_info.expiration_info, Some(expiration_info));
    ...
    assert_eq!(offer.order_info.price, price);
    assert_eq!(offer.order_info.creator, bidder);
    assert_eq!(offer.order_info.asset_recipient, Some(asset_recipient));
    assert_eq!(offer.order_info.finders_fee_bps, Some(finders_fee_bps));
    assert_eq!(offer.order_info.expiration_info, Some(expiration_info));
}
...
pub fn try_update_offer() {
    ...
    let response = router.execute_contract(bidder.clone(), marketplace.clone(), &update_offer, &[]);
    assert!(response.is_ok());
    ...
    let offer = router
        ...
        .unwrap()
        .unwrap();
    assert_eq!(offer.order_info.asset_recipient, Some(asset_recipient2));
    assert_eq!(offer.order_info.finders_fee_bps, Some(2000));
...
pub fn try_remove_offer() {
    ...
    let response = router.execute_contract(bidder.clone(), marketplace.clone(), &remove_offer, &[]);
    assert!(response.is_ok());
    ...
    // Reject offer as non NFT holder fails
    let token_id = "2";
    mint_and_set_ask(
        &mut router,
        &creator,
        ...
        &collection,
        token_id,
        &coin(native_denom_price_range.max.u128(), NATIVE_DENOM),
        &[config.listing_fee],
        None,
    );
    let price = coin(native_denom_price_range.min.u128(), NATIVE_DENOM);
    ...
    let reject_offer = ExecuteMsg::RejectOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        creator: bidder.to_string(),
    };
    let response =
        router.execute_contract(bidder2.clone(), marketplace.clone(), &reject_offer, &[]);
    assert_error(
        response,
        MarketplaceStdError::Unauthorized("sender is not owner or seller".to_string()).to_string(),
    );

    // Reject offer as NFT holder succeeds
    let reject_offer = ExecuteMsg::RejectOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        creator: bidder.to_string(),
    };
    let response = router.execute_contract(owner.clone(), marketplace.clone(), &reject_offer, &[]);
    assert!(response.is_ok());
    ...
    // Cannot remove offer that is not expired
    router
        .execute_contract(
            bidder.clone(),
            marketplace.clone(),
    ...
    let remove_expired_offer = ExecuteMsg::RemoveExpiredOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        creator: bidder.to_string(),
    };
    let response =
        router.execute_contract(bidder2, marketplace.clone(), &remove_expired_offer, &[]);
