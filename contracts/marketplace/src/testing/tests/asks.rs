...
use crate::{
    ...
    testing::{
        helpers::{
            ...
            nft_functions::{approve, burn, get_next_token_id_and_map, mint, mint_for, transfer},
        },
        setup::{
            setup_accounts::{
                setup_second_bidder_account, CREATION_FEE, INITIAL_BALANCE, MINT_PRICE,
            },
            setup_marketplace::{setup_marketplace, LISTING_FEE, MIN_EXPIRY},
            templates::{minter_template_owner_admin, standard_minter_template},
        },
    },
    ContractError,
};
...
fn try_query_sorted_asks() {
    ...
    // Another ask is made by the creator
    let set_ask = ExecuteMsg::SetAsk {
        collection: collection.to_string(),
    ...
    // Query asks by price
    let query_asks_msg = QueryMsg::AsksByPrice {
        collection: collection.to_string(),
        denom: NATIVE_DENOM.to_string(),
        query_options: None,
    };
    let asks: Vec<Ask> = router
        .wrap()
        .query_wasm_smart(marketplace.clone(), &query_asks_msg)
        .unwrap();
    assert_eq!(asks.len(), 3);
    assert_eq!(asks[0].price.amount.u128(), 109u128);
    assert_eq!(asks[1].price.amount.u128(), 110u128);
    assert_eq!(asks[2].price.amount.u128(), 111u128);

    // Query asks by price with start after
    let query_asks_msg = QueryMsg::AsksByPrice {
        collection: collection.to_string(),
        denom: NATIVE_DENOM.to_string(),
        query_options: Some(QueryOptions {
            descending: None,
            start_after: Some((asks[0].price.amount.u128(), asks[0].token_id.clone())),
            limit: None,
        }),
    };
    ...
    assert_eq!(asks[0].price.amount.u128(), 110u128);
    assert_eq!(asks[1].price.amount.u128(), 111u128);

    let reverse_query_asks_msg = QueryMsg::AsksByPrice {
        collection: collection.to_string(),
        denom: NATIVE_DENOM.to_string(),
        query_options: Some(QueryOptions {
    ...
    let asks: Vec<Ask> = router
        ...
        .unwrap();
    assert_eq!(asks.len(), 3);
    assert_eq!(asks[0].price.amount.u128(), 111u128);
    assert_eq!(asks[1].price.amount.u128(), 110u128);
    assert_eq!(asks[2].price.amount.u128(), 109u128);
    ...
    assert_eq!(asks.len(), 2);
    assert_eq!(asks[0].price.amount.u128(), 110u128);
    assert_eq!(asks[1].price.amount.u128(), 109u128);
}
...
fn max_set_ask_amount() {
    ...
    assert_eq!(
        ...
        ContractError::InvalidInput("price too high".to_string()).to_string()
    );
    ...
    assert_eq!(
        response.unwrap_err().source().unwrap().to_string(),
        ContractError::InvalidInput("price too low".to_string()).to_string()
    );
...
fn try_ask_with_finders_fee() {
    ...
    let set_bid_msg = ExecuteMsg::SetOffer {
        collection: collection.to_string(),
        token_id: token_id.to_string(),
        asset_recipient: None,
...
fn try_query_asks() {
    ...
    let asks: Vec<Ask> = router
        ...
        .query_wasm_smart(marketplace.clone(), &query_asks_msg)
        .unwrap();
    assert_eq!(asks[0].token_id, token_id.to_string());
    assert_eq!(asks[0].price.amount.u128(), 110);
    ...
    let query_asks_msg = QueryMsg::Asks {
        ...
        query_options: Some(QueryOptions {
            descending: None,
            start_after: Some(token_id.to_string()),
            limit: None,
        }),
    };
    let asks: Vec<Ask> = router
    ...
    let query_asks_msg = QueryMsg::Asks {
        ...
        query_options: Some(QueryOptions {
            descending: Some(true),
            start_after: Some((token_id + 1).to_string()),
            limit: None,
        }),
    };
    let asks: Vec<Ask> = router
...
fn try_query_asks_by_seller() {
    ...
    let marketplace = setup_marketplace(&mut router, fair_burn, owner.clone()).unwrap();
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    let collection = vt.collection_response_vec[0].collection.clone().unwrap();
    let _start_time = Timestamp::from_nanos(GENESIS_MINT_START_TIME);
    setup_block_time(&mut router, GENESIS_MINT_START_TIME, None);
    ...
    let set_ask = ExecuteMsg::SetAsk {
        collection: collection.to_string(),
        token_id: (token_id_0 + 2).to_string(),
        price: coin(111, NATIVE_DENOM),
        asset_recipient: None,
        reserve_for: None,
        finders_fee_bps: Some(0),
    ...
    let query_asks_msg = QueryMsg::AsksBySeller {
        ...
        query_options: Some(QueryOptions {
            descending: None,
            start_after: Some((collection.to_string(), "0".to_string())),
            limit: None,
        }),
    };
    let asks: Vec<Ask> = router
    ...
    let query_asks_msg = QueryMsg::AsksBySeller {
        ...
        query_options: Some(QueryOptions {
            descending: None,
            start_after: Some((collection.to_string(), (token_id_0 + 1).to_string())),
            limit: None,
        }),
    };
    let asks: Vec<Ask> = router
